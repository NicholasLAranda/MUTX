...
// MUTX: on startup load spork values from previous session if they exist in the sporkDB
...
int64_t GetSporkValue(int nSporkID)
{
    ...
    if (mapSporksActive.count(nSporkID)) {
		...
		if (nSporkID == SPORK_18_MANDATORY_UPDATE) r = SPORK_18_MANDATORY_UPDATE_DEFAULT;
...
int CSporkManager::GetSporkIDByName(std::string strName)
{
	...
	if (strName == "SPORK_18_MANDATORY_UPDATE") return SPORK_18_MANDATORY_UPDATE;
...
std::string CSporkManager::GetSporkNameByID(int id)
{
	...
	if (id == SPORK_18_MANDATORY_UPDATE) return "SPORK_18_MANDATORY_UPDATE";
