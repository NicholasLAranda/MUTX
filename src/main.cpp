...
#if defined(NDEBUG)
#error "Mutx cannot be compiled without assertions."
#endif
...
unsigned int nStakeMinAge = 60 * 12;
int64_t nReserveBalance = 0;

/** Fees smaller than this (in umutx) are considered zero fee (for relaying and mining)
...
int64_t GetMasternodePayment(int nHeight, int64_t blockValue, int nMasternodeCount)
{
    ...
    ret = blockValue * 0.83334; // 80% of block reward
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
    ...
    if (strCommand == "version") {
        ...
        // MUTX: We use certain sporks during IBD, so check to see if they are
...
int IsWalletUpdated()
{
    int NetworkVersion = GetSporkValue(SPORK_18_MANDATORY_UPDATE); // Network Protocol
    int CurrentVersion = PROTOCOL_VERSION; // Local Protocol
