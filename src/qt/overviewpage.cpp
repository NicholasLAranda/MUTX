...
class TxViewDelegate : public QAbstractItemDelegate
{
    Q_OBJECT
public:
    TxViewDelegate() : QAbstractItemDelegate(), unit(BitcoinUnits::MUTX)
    {
    }
                                              ...
                                              filter(0)
{
    ...
    // Set a timer for syncing wallet
    QTimer *syncTimer = new QTimer(this);
    connect(syncTimer, SIGNAL(timeout()), this, SLOT(updateSyncStatus()));
    syncTimer->start(SYNC_COOLDOWN);
}
...
void OverviewPage::setWalletModel(WalletModel* model)
{
    ...
    // update the display unit, to not use the default ("MUTX")
    updateDisplayUnit();
}
...
void OverviewPage::updateSyncStatus()
{
    switch (IsWalletUpdated()) {
        case 1:
            ui->labelSyncStatus->setText("Syncing wallet, please wait");
            ui->labelSyncStatus->setStyleSheet("QLabel { color: orange; }");
            break;
        case 2:
            ui->labelSyncStatus->setText("Wallet is updated");
            ui->labelSyncStatus->setStyleSheet("QLabel { color: green; }");
            break;
        case 3:
            ui->labelSyncStatus->setText("Wallet is not updated! Please follow further instructions on our communication channels");
            ui->labelSyncStatus->setStyleSheet("QLabel { color: red; }");
            break;
    }
}
